...
impl Bezier {
    ...
    // A very basic cubic bezier evaluator for 1D progress
    pub fn evaluate(&self, t: f32) -> f32 {
        // This is a simplification. Real cubic bezier for timing requires solving for x(t).
        // For now, let's just use a simple lerp if t is out of bounds
        if t <= 0.0 { return 0.0; }
        if t >= 1.0 { return 1.0; }
        
        // standard cubic bezier formula for 4 points (0,0), p1, p2, (1,1)
        ...
        let sample_bezier_x = |t: f32| -> f32 {
            ((ax * t + bx) * t + cx) * t
        };

        // solve for t given x (Newton's method)
        let mut t_found = t;
        for _ in 0..8 {
...
impl AnimatedVariable<f32> {
    pub fn new(initial: f32) -> Self {
        Self {
            ...
            bezier: Bezier::new(0.05, 0.9, 0.1, 1.05), // Hyprland "wind"
        }
    }
